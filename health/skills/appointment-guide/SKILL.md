...
2. **At most 3 questions, as a ladder.** Question 1 closes the decision and **connects with the next appointment on the route**. Questions 2 and 3 are **conditional** ("only if they say X"), never a list to read through. Each question carries a one-line "why" underneath in 9 pt, so the companion asks it with conviction rather than from memory.
