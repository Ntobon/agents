...
- **Index-first context recovery:** every folder keeps a `00 Índice general.md` even when it doesn't follow the full pattern yet (inventory mode: what each file is, date, transcription state, plus the timeline). Sessions recover context by reading CLAUDE.md → index → transcriptions, **never by rescanning the documents** — which is why every document that arrives or changes state is recorded in the index the same day.
...
| "pídeme una cita" / "agenda con el dermatólogo" / "qué médico me recomiendas de la red" | `appointment-booking` — coverage from the contract, directory cross-checked with public info and distance, one question (doctor + time window), booking via the insurer's WhatsApp bot or portal, email confirmation check, calendar event, records |
...
| "analiza el DICOM" / "haz un modelo 3D del TAC" / "dónde está el tumor" / a decision the images could inform | `imaging-3d` — local own reading of the study (CT/MRI) focused on one question, a Blender model with stills and a turntable video, and a single-file 3D viewer that explains the disease to the family (where it is and how big, with and without the lesion, flow through a narrowing, growth, treatment response, what surgery removes and leaves); the own reading never enters documents for doctors |
