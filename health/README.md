...
- **Appointment booking** (`appointment-booking`): end-to-end booking — what the plan covers (from the contract, since portals often show nothing), the insurer's directory cross-checked with public information on each doctor and distance from home, one question to the user (which doctor, which time window), booking through the insurer's WhatsApp bot or portal from the patient's own Chrome profile, verification of the confirmation email, and a calendar event with code and what to bring. Never types credentials, never pays, answers consent prompts with the most private option. Insurer-specific bot routes live in `references/` (Colsanitas included).
- **Appointment guide** (`appointment-guide`): the companion's 1-2 page sheet for a visit — the single decision the visit must end with (written in the record and reaching the next doctor on the route), three intervention moments, a ladder of at most three questions (only the first is unconditional), a scenario table with the exact sentence to get written for each possible answer, an exit checklist and what not to do. Built on the premise that doctors talk to the patient, have 15 minutes, and do not welcome twenty questions.
- **Private dashboards** (`internal-html-dashboard`): the owner's mobile-first living boards — status, agenda with persistent checkboxes, frank Q&A with sources, emergency signs — archived in Drive and republished to a stable URL. Designed to be glanced at: few words, big visuals (hero state, big figures, drawn schematics, to-scale bars) and a tooltip on every technical term. Ships a reference template (`assets/dashboard-reference.html`) with every component and fictitious data.
...
- **Channel monitoring** (`channel-monitoring`): a read-only browser round — on request or as a daily scheduled routine — over the channels where news arrives (the patient's mailbox, the insurer's portal, care-route portals). It compares against a per-channel watermark, downloads or screenshots what is new, archives it per the pattern, logs what was expected and did not arrive, and notifies only when something happened. Accounts, senders, URLs, and standing authorizations live in the patient's `CLAUDE.md`, never in the skill; it never types credentials, sends, files, or cancels anything.
- **Shift handoff** (`shift-handoff`): for a patient accompanied in shifts, turns the end-of-shift WhatsApp voice notes into the record — asks which chat to read and confirms which audios to download, transcribes them locally with Whisper (health audio never goes to an external service), archives raw audio plus transcript, runs a fixed extraction checklist with warning signs first, updates the care log, the day's clinical note and the index, and drafts the questions for the next shift, sent only on explicit approval.
- **3D imaging** (`imaging-3d`): turns a CT or MRI DICOM into an own reading focused on one question (slices, approximate measurements, what the image cannot show, concrete questions for a radiologist), a Blender model (stills and a turntable video for WhatsApp) and a single-file interactive viewer that answers the questions families always ask — where the lesion is and how big, what it would look like without it, how contents pass a narrowing, how it grows, when treatment response is known, what surgery removes and how it ends up. Everything local; a share copy for a trusted doctor; the own reading never goes into documents for doctors.
