...
description: Configure a new instance of the purchase-research agent (city, currency, language, HTML branding, and the research archive folder). Use when the user says "quiero instalarlo", "install this", "configura el agente de compras", "setup de compras", or when purchase-research can't find its configuration.
---
...
2. Write the config:
   - **With a working folder** (Claude Code): `local/config.json` with `{ "city", "currency", "language", "html_branding" }` and create `local/research/`. Explain that `local/` is private and never versioned.
